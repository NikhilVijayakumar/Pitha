...
if (-not (Test-Path $cargoToml)) {
    Write-Result "not_applicable" @("No Cargo.toml found - not a Rust project. Skipping lint.") @{ lint_exit_code = 0; lint_errors = 0 }
}

$lintConfigs = @("clippy.toml", ".clippy.toml", "rustfmt.toml", ".rustfmt.toml")
$configFound = @()
...
if (-not (Get-Command "cargo" -ErrorAction SilentlyContinue)) {
    Write-Result "error" @("cargo not installed - cannot run lint") @{ lint_exit_code = -1; lint_errors = 0 }
}
