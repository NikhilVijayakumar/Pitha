...
for mod in "${DECLARED_MODULES[@]}"; do
    ...
    done < <(find "$mod_dir" -type f \( -name "*.rs" -o -name "*.ts" -o -name "*.js" -o -name "*.py" -o -name "*.go" -o -name "*.java" -o -name "*.cs" \) -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/__pycache__/*" -not -path "*/vendor/*" -print0 2>/dev/null)
done
